...
class DashboardContextTest(TestCase):
	...
	def test_annual_activity_chart_uses_activity_table_monthly_average(self):
		...
		self.assertEqual(rows['Jan']['moyenne'], 3238390)
		self.assertEqual(rows['Jan']['moyenne_plus_10'], 3562229)
		self.assertEqual(rows['Jan']['moyenne_moins_10'], 2914551)
		self.assertEqual(rows['Jan']['moyenne_mensuelle'], 3238390)
		self.assertEqual(rows['Jan']['annees_actives'], 2)
		self.assertEqual(rows['Jan']['annee'], 3432813)
		self.assertEqual(rows['Jan']['ecart_pct'], 6.0)
		self.assertTrue(rows['Jan']['ecart_positive'])
		self.assertEqual(rows['Fev']['moyenne'], 8780436)
		self.assertEqual(rows['Fev']['moyenne_plus_10'], 9658480)
		self.assertEqual(rows['Fev']['moyenne_moins_10'], 7902392)
		self.assertEqual(rows['Fev']['moyenne_mensuelle'], 5542046)
		self.assertEqual(rows['Fev']['annees_actives'], 2)
		self.assertEqual(rows['Fev']['annee'], 8430400)
		self.assertEqual(rows['Fev']['ecart_pct'], -4.0)
		self.assertEqual(rows['Fev']['ecart_label'], '-4.0%')
		self.assertFalse(rows['Fev']['ecart_positive'])
		self.assertEqual(rows['Mar']['moyenne'], 12740383)
		self.assertEqual(rows['Mar']['moyenne_mensuelle'], 3959947)
		self.assertEqual(rows['Mar']['annees_actives'], 2)
		self.assertEqual(rows['Mar']['annee'], 12964297)
		self.assertEqual(rows['Mar']['ecart_pct'], 1.8)
		self.assertTrue(rows['Mar']['ecart_positive'])
		self.assertEqual(rows['Avr']['moyenne'], 17093115)
		self.assertEqual(rows['Avr']['moyenne_mensuelle'], 4352732)
		self.assertEqual(rows['Avr']['annees_actives'], 2)
		self.assertEqual(rows['Avr']['annee'], 17686174)
		self.assertEqual(rows['Avr']['ecart_pct'], 3.5)
		self.assertTrue(rows['Avr']['ecart_positive'])
		self.assertTrue(rows['Avr']['current_visible'])
		self.assertFalse(rows['Mai']['current_visible'])
		self.assertTrue(context['activite_annuelle_points_moyenne_plus_10'])
		self.assertTrue(context['activite_annuelle_points_moyenne_moins_10'])
